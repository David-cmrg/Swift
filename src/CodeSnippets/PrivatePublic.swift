...
class MovieManager {
    // Public var, no matter if "public" is written infront of it or not
    // Public means we can both get and set from outside of the Object itself
    public var movie1 = MovieModel(title: "Avatar", genre: .action, isFavorite: false)
    
    // Private means we can NOT get or set from outside of class
    private var movie2 = MovieModel(title: "Step Brothers", genre: .comedy, isFavorite: false)

    // Private(set) Read is public and the set is private
    // We can read the movie3 data but cannot change or update it
...
let someValue = manager.movie1 // Here we are getting the Value of Movie1



// Option 1: 
// manager.movie1 = manager.movie1.updateFavoriteStatus(newValue: true) // Here we are updating the Value of Movie1

// Option 2:
// manager.movie1.updateFavoriteStatus2(newValue: true) // Here we are updating the Value of Movie1

// Update Movie3 with func for "private (set) var"
